...
Accidental Haiku Detector — Word-Only Edition
- Detects 5-7-5 and 3-5-3 haikus using a word-by-word sliding window
- Pulls top 200 Gutenberg books and filters 100 valid English ones
- Outputs grouped markdown zine
"""
...
def extract_haikus(path):
    with open(path, encoding="utf-8", errors="ignore") as f:
        text = clean_text(f.read())
    patterns = [[5, 7, 5], [3, 5, 3]]
    return find_word_haikus(text, patterns)
...
def main():
    ...
    for fname in tqdm(books, desc="Scanning"):
        ...
        haikus = extract_haikus(file_path)
